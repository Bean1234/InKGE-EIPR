...
class InGramEntityLayer(nn.Module):
    def __init__(self, dim_in_ent, dim_out_ent, dim_rel, bias = True, num_head = 8):
        ...
        self.rel_head = nn.Linear(dim_rel,dim_out_ent, bias = bias)
        self.rel_tail = nn.Linear(dim_rel, dim_out_ent, bias = bias)
        self.pro=nn.Linear(dim_out_ent*3,dim_out_ent)
        self.node_proj = nn.Linear(dim_in_ent+4, dim_in_ent)
    ...
    def param_init(self):
        ...
        nn.init.xavier_normal_(self.rel_head.weight, gain = nn.init.calculate_gain('relu'))
        nn.init.xavier_normal_(self.rel_tail.weight, gain=nn.init.calculate_gain('relu'))
        nn.init.xavier_normal_(self.pro.weight,gain=nn.init.calculate_gain('relu'))
        nn.init.xavier_normal_(self.node_proj.weight, gain=nn.init.calculate_gain('relu'))

        if self.bias:
            nn.init.zeros_(self.attn_proj.bias)
            nn.init.zeros_(self.aggr_proj.bias)
    
    def forward(self, emb_ent, emb_rel, triplets,emb_ent_dict):
        num_ent = len(emb_ent)
        num_rel = len(emb_rel)
        ...
        emb_head_ent = self.rel_head(emb_rel[rel_idxs])
        emb_tail_ent = self.rel_tail(emb_rel[rel_idxs])

        ent_emb_new_head = torch_scatter.scatter_mean(dim=0, index=triplets[..., 0], src=emb_head_ent)
        ent_emb_new_tail = torch_scatter.scatter_mean(dim=0, index=triplets[..., 2], src=emb_tail_ent)
       
        ent_emb_new = (ent_emb_new_head+ent_emb_new_tail+emb_ent)/3
        emb_ent = self.act(ent_emb_new)
    

        emb_ent_att = self.node_proj(torch.cat([emb_ent, emb_ent_dict['ent_node_struct_emb'].to(emb_ent.device)],dim=-1))




        concat_mat_att = torch.cat([emb_ent_att[tail_idxs], emb_ent_att[head_idxs], \
                                    emb_rels], dim = -1)
...
class InGramRelationLayer(nn.Module):
    def __init__(self, dim_in_rel, dim_out_rel, num_bin, bias = True, num_head = 8):
        ...
        self.attn_proj = nn.Linear(2*dim_out_rel+16, dim_out_rel, bias = bias)
        ...
        self.node_proj = nn.Linear(dim_in_rel+4, dim_in_rel, bias = bias)
        self.type_proj = nn.Linear(4,16)
    ...
    def param_init(self):
        ...
        nn.init.xavier_normal_(self.aggr_proj.weight, gain = nn.init.calculate_gain('relu'))
        nn.init.xavier_normal_(self.node_proj.weight, gain = nn.init.calculate_gain('relu'))
        nn.init.xavier_normal_(self.type_proj.weight, gain = nn.init.calculate_gain('relu'))
    ...
    def forward(self, emb_rel, relation_triplets, emb_rel_dict):
        num_rel = len(emb_rel)
        rel_node_struct_emb = emb_rel_dict['rel_node_struct_emb']
        rel_type = emb_rel_dict['A_type']

        
        head_idxs = relation_triplets[..., 0]
        tail_idxs = relation_triplets[..., 1]
      
        type_feature = self.act(self.type_proj(rel_type.to(head_idxs.device)[head_idxs, tail_idxs]))

     

        concat_mat = torch.cat([emb_rel[head_idxs], emb_rel[tail_idxs],type_feature], dim = -1)
...
class InGram(nn.Module):
    def __init__(self, dim_ent, hid_dim_ratio_ent, dim_rel, hid_dim_ratio_rel, num_bin, num_layer_ent=2, num_layer_rel=2, \
        ...
        self.act = nn.ReLU()
        self.score_emb = nn.Linear(dim_rel*3,dim_rel)
    ...
    def param_init(self):
        ...
        nn.init.xavier_normal_(self.score_emb.weight, gain=nn.init.calculate_gain('relu'))
    ...
    def forward(self, emb_ent, emb_rel, triplets, relation_triplets):
        layer_emb_ent = emb_ent['init_emb_ent']

        layer_emb_ent = self.ent_proj1(layer_emb_ent)
        emb_rel_init =  emb_rel['init_emb_rel']
        layer_emb_rel = self.rel_proj1(emb_rel_init)
        
        for layer_idx, layer in enumerate(self.layers_rel):
            layer_emb_rel = layer(layer_emb_rel, relation_triplets, emb_rel) + \
        ...
        for layer_idx, layer in enumerate(self.layers_ent):
            ...
            layer_emb_ent = layer(layer_emb_ent, layer_emb_rel, triplets,emb_ent) + \
    ...
    def score(self, emb_ent, emb_rel, triplets,G_nx):
