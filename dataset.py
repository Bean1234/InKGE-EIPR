...
class TestNewData():
	def __init__(self, path, data_type = "valid"):
		...
		self.rel2id = None
		self.G_nx = None
		self.id2ent, self.id2rel, self.msg_triplets, self.sup_triplets, self.filter_dict,self.G_nx = self.read_triplet()
		self.num_ent, self.num_rel = len(self.id2ent), len(self.id2rel)
	...
	def read_triplet(self):
		...
		return id2ent, id2rel, np.array(msg_triplets), np.array(sup_triplets), filter_dict,G_nx
